	...
	BEGIN TRY
		...
		SELECT 
			...
			CASE
				...
				ELSE CAST(CAST(sls_order_dt AS VARCHAR) AS DATE)	-- Handling invalid data and type casting
			END AS sls_order_dt,
			CASE
			...
			CASE 
				...
				ELSE sls_sales
			END AS sls_slaes,		-- Recalculate sales if original values is missing or incorrect
			sls_quantity,
			CASE
		...
		SELECT
			...
			CASE
				...
				ELSE TRIM(cntry)
			END AS cntry			-- Normalize country values and handle unknowns
		...
		INSERT INTO silver.erp_px_cat_g1v2 (
			...
			subcat,
			maintenance
		)				-- Table quality is good, no transformations
		SELECT 
			id,
			cat,
			subcat,
